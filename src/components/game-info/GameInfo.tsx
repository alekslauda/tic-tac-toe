...
export default function GameInfo({ turn, gameState, marker, resetGame, nextMove }: GameInfoProps) {
  ...
  const templateFactory = function (): any[] {
    ...
    if (gameState === GameState.GAME_ON) {
      template.push(
        (<li key={`'game_on_1_${ new Date().getTime() }`}>First plays {turn}</li>), 
        (<li key={`'game_on_2_${ new Date().getTime() }`}>{Game.PLAYER1} plays with: {marker[0]}</li>), 
        (<li key={`'game_on_3_${ new Date().getTime() }`}>{Game.PLAYER2} plays with: {marker[1]}</li>)
      );
    } else if(gameState === GameState.DRAW) {
      resetGamePossibility = true;
      template.push(
        (<li key={`'game_draw_1_${ new Date().getTime() }`}>The End! Game is draw</li>),
        (<li key={`'game_draw_2_${ new Date().getTime() }`}>Nobody wins! <strong>:X :X :X</strong></li>)
      )
    } else if(gameState === GameState.WIN) {
      resetGamePossibility = true;
      template.push(
        (<li key={`'game_win_1_${ new Date().getTime() }`}>We have a winner!</li>), 
        (<li key={`'game_win_2_${ new Date().getTime() }`}>{nextMove} wins the game! <strong>:) :) :)</strong></li>),
      )
